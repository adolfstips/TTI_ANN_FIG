...
# Make a simplified K-fold cross validation MLP70SPAT
...
# Update: Secchi added to Remote Sensing!
var_com = list(
...
n_splits <- 10 # Number of random spatial block splits to test
...
for (simul in seq_along(var_com)) {
  ...
  for (k in 1:n_splits) {
    ...
    # 3a. Unique Spatial Split & Scaling per iteration
    set.seed(123 + k) # Ensures a different split each time
    dataall <- dataall_raw
    
    for (p in vari) dataall[[p]] <- scale(dataall[[p]]) # Scale
    ...
    # 3b. Tensor Prep (Standard 2D for MLP)
    ...
    # 3c. MLP Model 
    model <- nn_sequential(
      nn_linear(nvari, 128), nn_relu(), nn_dropout(p = 0.2),
      nn_linear(128, 64), nn_relu(), nn_dropout(p = 0.2),
      nn_linear(64, 1)
    )
    ...
    # --- 3d. Dynamic Training Loop with Early Stopping ---
    batch_size <- 2048
    ...
    for (epoch in 1:150) {
      ...
      # Forward Pass (Validation check for Early Stopping)
      model$eval()
      val_loss <- 0
      with_no_grad({
    ...
    # Reload the absolute best weights achieved during this fold
...
cat("K-fold cross validation MLP70SPAT\n")
cat("==================================================\n")

# Compute 95% CI (1.96 * Standard Error)
